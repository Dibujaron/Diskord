...
suspend fun dispatchEvent(eventListener: EventListener, event: DiscordEvent, data: JsonElement) {
    ...
    when (event) {
        DiscordEvent.READY -> eventListener.onReady(
            defaultJson.decodeFromJsonElement(
                Ready.serializer(),
                data
            )
        )
        DiscordEvent.RESUMED -> eventListener.onResumed(
            defaultJson.decodeFromJsonElement(
                Resumed.serializer(),
                data
            )
        ...
        DiscordEvent.GUILD_UPDATE -> eventListener.onGuildUpdate(
            defaultJson.decodeFromJsonElement(
                Guild.serializer(),
                data
            )
        )
        DiscordEvent.GUILD_DELETE -> eventListener.onGuildDelete(
            defaultJson.decodeFromJsonElement(
                Guild.serializer(),
                data
            )
        )
        DiscordEvent.GUILD_BAN_ADD -> eventListener.onGuildBanAdd(
            defaultJson.decodeFromJsonElement(
                GuildBan.serializer(),
                data
            )
        ...
        DiscordEvent.USER_UPDATE -> eventListener.onUserUpdate(
            defaultJson.decodeFromJsonElement(
                User.serializer(),
                data
            )
